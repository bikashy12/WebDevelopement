...
export default function Contacts(props) {
  ...
  const ValidationProcess = (props) => {
    ...
    //   });
    // } 
    else if (mobile1 != '' && mobile1 != null && regex.test(mobile1) == false) {
      setErrors({
        ...errors,
      ...
      <View
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            //   props.onChange(item);
            console.log('*********************');
            let wizard_details = JSON.parse(item.wizard_details);
            let character_data = JSON.parse(item.character);
            //   console.log(wizard_details.gender);
            console.log(item);
            //   setformdetails(item);
            var styles_id = null;
            var character_id = null;
            var personas_id = null;
            if (wizard_details != null) {
              styles_id =
                wizard_details.activeLifeStyle != null
                  ? wizard_details.activeLifeStyle
                  : null;
              character_id =
                wizard_details.characterTrait != null
                  ? wizard_details.characterTrait
                  : null;
              personas_id =
                wizard_details.persona != null ? wizard_details.persona : null;
            }
            setparams({
              ...params,
              ['contact_id']: item.id,
              ['gender']: item.gender!=null?item.gender:params['gender'],
        ...
        <TouchableOpacity
          ...
          <Icon name="delete" size={20} color="#B60612" />
        </TouchableOpacity>
      </View>
    );
  };
